...
var TourDataProvider = function (tour, year) {
  ...
  /**
   * [djb 01/07/2024] reverted back to pgatour as the golf channel site
   *                 is no longer providing the data.  We have data prior
   *                 to 2023 with golf channel, so still use that.  For
   *                  2023 and beyond, we use the pgatour site.
  ...
  this.getEvent = function (eventid, details) {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        "getting tour info for " + tourName + " " + year + " eventid " + eventid
      );
